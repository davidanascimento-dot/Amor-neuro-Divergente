document.addEventListener('DOMContentLoaded', () => {
    ...
    const devToolsSidebar = document.getElementById('devToolsSidebar');
    const mainWrapper = document.getElementById('mainWrapper');
    const assistantTextArea = document.getElementById('assistantTextArea');
    const chips = document.querySelectorAll('.suggestion-chip');

    if (menuToggleBtn && devToolsSidebar && mainWrapper) {
        menuToggleBtn.addEventListener('click', () => {
            devToolsSidebar.classList.toggle('collapsed');
            mainWrapper.classList.toggle('expanded');
        });

        closeSidebarBtn.addEventListener('click', () => {
            devToolsSidebar.classList.add('collapsed');
            mainWrapper.classList.add('expanded');
        });
    }

    // Evento de clique nos chips de sugestão
    if (chips.length && assistantTextArea) {
        chips.forEach(chip => {
            chip.addEventListener('click', function() {
                assistantTextArea.value = this.innerText;
